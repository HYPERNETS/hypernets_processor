    ...
    class SpectrumHeader:
        class AccelStats:
            ...
            def parse_raw(cls, data):
                ...
                a.mean_x, a.std_x, a.mean_y, a.std_y, a.mean_z, a.std_z = struct.unpack(
                    "<hhhhhh", data
                )
                return a
        ...
        def parse_header(cls, data):
            ...
            h.spectrum_type = Spectrum.SpectrumHeader.SpectrumType.parse_raw(
                h.spectrum_type
            )
    ...
    def parse_raw(cls, data, save_raw=False, slot=0):
        ...
        if save_raw:
            save_path = os.path.join(
                ...
                "specs",
                "run1",
                time.strftime("%Y_%m_%d_T%H%M%S_")
                + s.header.spectrum_type.optics.name
                + "_"
                + str(slot)
                + ".bin",
            )
