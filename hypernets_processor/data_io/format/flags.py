FLAG_COMMON = ["saturation", "nonlinearity", "bad_pointing", "placeholder1", "lon_default", "lat_default",
               "outliers", "L0_thresholds", "L0_discontinuity", "dark_outliers", "vza_irradiance",
               "clear_sky_irradiance"]

FLAG_WATER = ["angles_missing", "lu_eq_missing", "fresnel_angle_missing", "fresnel_default", "temp_variability_ed",
                 "temp_variability_lu", "min_nbred", "min_nbrlu", "min_nbrlsky", "def_wind_flag", "simil_fail",
              "ld_ed_clearsky_failing"]

FLAG_LAND = []
