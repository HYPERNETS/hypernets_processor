...
CAL_VARIABLES = {"wavelengths": {"dim": [CD_DIM,WL_DIM],
                 ...
                 # "u_rel_random_gains": {"dim": [WL_DIM],
                 #                           "dtype": np.uint32,
                 #                           "attributes": {"standard_name": "random relative uncertainty on gains",
                 #                                          "long_name": "random relative uncertainty on gains (calibration coefficients)",
                 #                                          "units": "-"},
                 #                           "encoding": {'dtype': np.uint32, "scale_factor": 0.001, "offset": 0.0}},
                 "u_rel_systematic_indep_gains": {"dim": [CD_DIM,WL_DIM],
                                              "dtype": np.float32,
                 ...
                 "u_rel_systematic_corr_rad_irr_gains": {"dim": [CD_DIM,WL_DIM],
                                           "dtype": np.float32,
...
L1A_RAD_VARIABLES = {"u_rel_random_radiance": {"dim": [WL_DIM, SCAN_DIM],
                                           "dtype": np.uint16,
...
L1A_IRR_VARIABLES = {"u_rel_random_irradiance": {"dim": [WL_DIM, SCAN_DIM],
                                             "dtype": np.uint16,
...
L1B_RAD_VARIABLES = {"u_rel_random_radiance": {"dim": [WL_DIM, SERIES_DIM],
                                           "dtype": np.float32,
                                           "attributes": {"standard_name": "random relative uncertainty on radiance",
                                                          "long_name": "random relative uncertainty on upwelling radiance",
                                                          "units": "-"},
...
L1B_IRR_VARIABLES = {"u_rel_random_irradiance": {"dim": [WL_DIM, SERIES_DIM],
                                             "dtype": np.float32,
                                             "attributes": {"standard_name": "random relative uncertainty on irradiance",
                                                            "long_name": "random relative uncertainty on downwelling irradiance",
                                                            "units": "-"},
...
W_L1C_VARIABLES = {"u_rel_random_downwelling_radiance": {"dim": [WL_DIM, Lu_SCAN_DIM],
                                                     "dtype": np.float32,
                                                     "attributes": {"standard_name": "random relative uncertainty on radiance",
                                                                    "long_name": "random relative uncertainty on upwelling radiance",
                                                                    "units": "-"},
...
L_L2A_REFLECTANCE_VARIABLES = {"u_rel_random_reflectance": {"dim": [WL_DIM, SERIES_DIM],
                                                        "dtype": np.float32,
...
W_L1C_STD_VARIABLES = {"irradiance_std": {"dim": [WL_DIM],
                                               "dtype": np.float32,
                                               "attributes": {"standard_name": "irradiance_std",
                                                              "long_name": "Standard deviation from irradiance scans ",
                                                              "reference": "SYSTEM_HEIGHT_DEPLOYEMENT",
                                                              "units": "mW m^-2 nm^-1",
                                                              "preferred_symbol": "ed_std"}},

                               "downwelling_radiance_std": {"dim": [WL_DIM],
                                                          "dtype": np.float32,
                                                          "attributes": {
                                                              "standard_name":
                                                                  "water_leaving_radiance_std",
                                                              "long_name": "",
                                                              "reference": "",
                                                              "units": "mW m^-2 nm^-1 sr^-1",
                                                              "preferred_symbol": "ld_std"}},
                              }


W_L2A_STD_VARIABLES = {"reflectance_std": {"dim": [WL_DIM, Lu_SCAN_DIM],
                                               "dtype": np.float32,
                                               "attributes": {"standard_name": "reflectance_std",
                                                              "long_name": "Standard deviation from reflectance scans ",
                                                              "reference": "SYSTEM_HEIGHT_DEPLOYEMENT",
                                                              "units": "-",
                                                              "preferred_symbol": "rhow_std"}},

                               "water_leaving_radiance_std": {"dim": [WL_DIM, Lu_SCAN_DIM],
                                                          "dtype": np.float32,
                                                          "attributes": {
                                                              "standard_name":
                                                                  "water_leaving_radiance_std",
                                                              ...
                                                              "reference": "",
                                                              "units": "mW m^-2 nm^-1 sr^-1",
                                                              "preferred_symbol": "lw_std"}},
                               "reflectance_nosc_std": {"dim": [WL_DIM, Lu_SCAN_DIM],
                                                    "dtype": np.float32,
                                                    "attributes": {"standard_name": "water_leaving_reflectance_nosc_std",
                                                                   "long_name": "Standard deviation from reflectance of the water column at the "
                                                                                "surface without correction for the NIR "
                                                                                "similarity spectrum "
                                                                                "(see Ruddick et al., 2006)",
                                                                   "units": "-",
...
for variable in W_L2A_STD_VARIABLES.keys():
    W_L2A_STD_VARIABLES[variable]["dim"] = [d if d != Lu_SCAN_DIM else SERIES_DIM
                                                    for d in W_L2A_STD_VARIABLES[variable]["dim"]]

# L_L2B_REFLECTANCE_VARIABLES - Reflectance variables required for L2A land data product
L_L2B_REFLECTANCE_VARIABLES = {"u_rel_random_reflectance": {"dim": [WL_DIM, SERIES_DIM],
                                                        "dtype": np.float32,
                                                        "attributes": {"standard_name": "u_rel_random_reflectance",
                                                                       "long_name": "Random reflectance relative uncertainty",
                                                                       "units": "%"},
...
VARIABLES_DICT_DEFS: Any = {"L0_RAD": {**COMMON_VARIABLES_SCAN, **C_QUALITY_SCAN, **L0_RAD_VARIABLES},
                            ...
                            "W_L1C": {**COMMON_VARIABLES_Lu_SCAN, **W_QUALITY_SCAN,
                                      **W_L1C_VARIABLES, **W_L1C_REFLECTANCE_VARIABLES, **W_L1C_STD_VARIABLES},
                            "L_L2A": {**COMMON_VARIABLES_SERIES, **L_QUALITY, **L_L2A_REFLECTANCE_VARIABLES},
                            "W_L2A": {**COMMON_VARIABLES_SERIES, **W_QUALITY_SERIES, **W_L2A_REFLECTANCE_VARIABLES, **W_L2A_STD_VARIABLES},
