...
class StepCombine(MeasurementFunction):
    def meas_function(self, wavs_vis, rad_VIS, wavs_swir, rad_SWIR, wav_step):
        ...
        if len(wavs_vis.shape) > 1:
            rad = np.concatenate(
                (
                    rad_VIS[np.where(wavs_vis[:, 0] <= wav_step[0])[0], :],
                    rad_SWIR[np.where(wavs_swir[:, 0] > wav_step[0])[0], :],
                ),
                axis=0,
            )
        else:
            rad = np.concatenate(
                (
                    rad_VIS[np.where(wavs_vis <= wav_step)],
                    rad_SWIR[np.where(wavs_swir > wav_step)],
                ),
                axis=0,
            )
        return rad
