...
class RhymerHypstar:
    ...
    def get_wind(self, l1b):
        ...
        for i in range(len(l1b.scan)):
            if self.wind_ancillary is not None:
                isodate = datetime.utcfromtimestamp(l1b['acquisition_time'][i].values).strftime('%Y-%m-%d')
                isotime = datetime.utcfromtimestamp(l1b['acquisition_time'][i].values).strftime('%H:%M:%S')
                anc_wind = RhymerAncillary.get_wind(isodate, lon, lat, isotime=isotime)
        ...
        for i in range(len(l1b.scan)):
            ...
            if self.fresnel_option == 'Mobley':
                if (fresnel_sza[i] is not None) & (fresnel_raa[i] is not None):
                    sza_ = min(fresnel_sza[i], 79.999)
                    rhof = RhymerProcessing().mobley_lut_interp(fresnel_sza[i], fresnel_vza[i], fresnel_raa[i],
                                                                wind=wind[i],
                                                                rholut=rholut)
                else:
                    # add a quality flag!
    ...
    def process_l1b(self, L1a_rad, L1a_irr):
        ...
        L1a_rad = self.qc_scan(L1a_rad, measurandstring="radiance", verbosity=10)
        L1a_irr = self.qc_scan(L1a_irr, measurandstring="irradiance", verbosity=10)
        # QUALITY CHECK: MIN NBR OF SCANS -> ASSIGN FLAG
        L1a_uprad, L1a_downrad, L1a_irr = self.cycleparse(L1a_rad, L1a_irr, verbosity=10)
        ...
        L1b = self.intp.interpolate_l1b_w(L1a_uprad, L1b_downrad, L1b_irr, "WaterNetworkInterpolationLinear")
