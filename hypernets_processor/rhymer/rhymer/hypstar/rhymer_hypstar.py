...
class RhymerHypstar:
    ...
    def checkazimuths(self, rad):
        ...
        azis = np.unique(
            [
                round(
                    normalizedeg(
                        float(rad["pointing_azimuth_angle"].sel(scan=i).values), 0, 360
                    )
                )
                for i in uprad
            ]
    ...
    def selectazimuths(self, rad, irr, a):
        ...
        for i in rad["scan"]:
            ...
            sena_rad = round(
                normalizedeg(float(scani_["pointing_azimuth_angle"].values))
            )
        ...
        ra = (
            np.nanmean(rad["pointing_azimuth_angle"].sel(scan=scanaz_rad))
            - np.nanmean(rad["solar_azimuth_angle"].sel(scan=scanaz_rad))
        ) % 360

        return rad.sel(scan=scanaz_rad), irr.sel(scan=scanaz_irr), ra

    def cycleparse(self, rad, dataset_l1b):
        ...
        if (protocol != "WaterNetworkProtocol") and (self.context.logger is not None):
            ...
            for i in lu["scan"]:
                scani = lu.sel(scan=i)
                sena = scani["pointing_azimuth_angle"].values
                senz = scani["viewing_zenith_angle"].values
                if self.context.logger is not None:
            ...
            for i in senz_lu:
                if (
                    np.in1d(
                        np.linspace(np.round(i) - 2, np.round(i) + 2, 5),
                        np.round(senz_lsky),
                    ).sum()
                    >= 1
                ):
    ...
    def process_l1c_int(self, l1a_rad, L1b_irr):
        ...
        L1a_uprad, L1a_downrad, dataset_l1flags = self.cycleparse(
            l1a_rad, dataset_l1flags
        )

        self.qual.check_valid_irradiance(L1b_irr)

        if (
            self.qual.qc_illumination(L1a_downrad, "radiance")
            > self.context.get_config_value("ld_variability_percent") / 100 / 2
        ):  # /2 is to account for difference between calculating relative std and % difference (assuming values at start and values at end are similar):
            self.context.logger.info(
                "Non constant downwelling radiance for sequence {}".format(
                    L1a_downrad.attrs["sequence_id"]
                )
            )
