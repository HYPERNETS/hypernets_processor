...
class RhymerProcessing:
    ...
    def mobley_lut_interp(self, ths, thv, paa, wind=2.0):
        ...
        it1 = self.rhymershared.interp3d(
            rholut["data"][wind_br[0], :, :, :], ths_id, thv_id, phi_id
        )
        it2 = self.rhymershared.interp3d(
            rholut["data"][wind_br[1], :, :, :], ths_id, thv_id, phi_id
        )

        ## and weigh to wind speed
        rint = it2 * wind_w + it1 * (1.0 - wind_w)
        return rint
    ...
    def mobley_lut_read(self):
        ...
        ifile = "{}/{}".format(
            self.dir_path, "../rhymer/data/Shared/Mobley/{}.txt".format(vf)
        )
        ...
        with open(ifile, "r") as f:
            for li, line in enumerate(f.readlines()):
                ...
                if "rho for" in line:
                    ...
                    ws = [
                        sp[v]
                        for v in range(1, len(sp) - 1)
                        if (sp[v - 2] == "SPEED") & (sp[v + 1] == "m/s")
                    ]
                    ws = float(ws[0])
                    ths = [
                        sp[v]
                        for v in range(1, len(sp) - 1)
                        if (sp[v - 2] == "THETA_SUN") & (sp[v + 1] == "deg")
                    ]
                    ths = float(ths[0])
                    cur = {"ws": ws, "ths": ths}
                else:
    ...
    def similarity_read(self, ifile=None):
        ...
        if ifile is None:
            ifile = "{}/Shared/REMSEM/similarityspectrumtable.txt".format(
                self.context.get_config_value("rhymer_data_dir")
            )
        ...
        with open(ifile, "r") as f:
            for i, line in enumerate(f.readlines()):
                ...
                if i == 0:
                    ...
                    j = 0
                    while j < len(sp):
                        ss_data["wave"] = np.append(
                            ss_data["wave"], float(sp[j]) / 1000.0
                        )
