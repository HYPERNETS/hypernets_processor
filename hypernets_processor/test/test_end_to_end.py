...
def setup_test_files(context):
    ...
    ds_l1a_rad = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "scan": n_series * 10}, "L_L1A_RAD"
    )
    ds_l1a_irr = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "scan": n_series * 10}, "L_L1A_IRR"
    )

    ds_l1b_rad = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "series": n_series}, "L_L1B_RAD"
    )
    ds_l1b_irr = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "series": n_series}, "L_L1B_IRR"
    )

    ds_l2a = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "series": n_series}, "L_L2A"
    )
    ds_l2a_avg = dsb.create_ds_template(
        {"wavelength": N_WAVELENGTHS_2, "series": n_series}, "L_L2A"
    )
    ...
    ds_l0_bla["digital_number"].values = np.zeros(
        ds_l0_bla["digital_number"].values.shape
    )
...
class TestEndToEnd(unittest.TestCase):
    ...
    def test_end_to_end(self):
        ...
        context.set_config_value(
            "plotting_directory", os.path.join(tmpdir, "out/plots/")
        )
        ...
        L1a_rad = cal.calibrate_l1a(
            "radiance", test_l0_rad, test_l0_bla, calibration_data_rad
        )
        L1a_irr = cal.calibrate_l1a(
            "irradiance", test_l0_irr, test_l0_bla, calibration_data_irr
        )
        ...
        np.testing.assert_allclose(
            test_l2a["reflectance"].values,
            L2a["reflectance"].values,
            rtol=0.19,
            equal_nan=True,
        )
        np.testing.assert_allclose(
            np.nansum(test_l2a["reflectance"].values),
            np.nansum(L2a["reflectance"].values),
            rtol=0.05,
            equal_nan=True,
        )
        np.testing.assert_allclose(
            np.nansum(test_l2a_avg["reflectance"].values),
            np.nansum(L2a["reflectance"].values),
            rtol=0.001,
            equal_nan=True,
        )
