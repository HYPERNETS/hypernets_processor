...
IDs.to_csv(r'T:\ECO\EOServer\data\insitu\hypernets\post_processing_qc\joe\clean_irr_IDs.csv')
szas = df_clean['SZA']
...
def check(data, tolerance, wavelength):
    ...
    for i in range(data.shape[0]):
        aod_0 = find_nearest_to_wav(modelled_data_read_and_interp(szas[i], 0.0), wav, wavelength)
        aod_1 = find_nearest_to_wav(modelled_data_read_and_interp(szas[i], 0.1), wav, wavelength)
        aod_2 = find_nearest_to_wav(modelled_data_read_and_interp(szas[i], 0.2), wav, wavelength)
        aod_3 = find_nearest_to_wav(modelled_data_read_and_interp(szas[i], 0.3), wav, wavelength)
...
def tol_checks(data, tol_start, tol_stop, tol_step, wavelength):
    ...
    return np.vstack((tol, aod_0_outs, aod_0_good, aod_1_outs, aod_1_good, aod_2_outs, aod_2_good, aod_3_outs, aod_3_good))
...
def wav_checks(data, tol, wav_start, wav_stop, wav_step):
    ...
    return np.vstack((wav, aod_0_outs, aod_0_good, aod_1_outs, aod_1_good, aod_2_outs, aod_2_good, aod_3_outs, aod_3_good))
