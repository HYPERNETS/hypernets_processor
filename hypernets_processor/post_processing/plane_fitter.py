...
#read and bin data
data_19 = pd.read_csv(r'T:/ECO/EOServer/data/insitu/hypernets/post_processing_qc/joe/GHNA_2022_plane_definition.csv')
data_0875 = pd.read_csv(r'T:/ECO/EOServer/data/insitu/hypernets/post_processing_qc/joe/prelim_cc0875.csv')
data_unmasked = pd.read_csv(r'T:\ECO\EOServer\data\insitu\hypernets\post_processing_qc\joe\GHNA_2022_prelim_and_raa10.csv')
...
data_19_list = [data_19_30_90, data_19_90_150, data_19_150_210, data_19_210_270, data_19_270_330, data_19_330_30]
...
data_0875_list = [data_0875_30_90, data_0875_90_150, data_0875_150_210, data_0875_210_270, data_0875_270_330, data_0875_330_30]
...
data_unmasked_list = [data_unmasked_30_90, data_unmasked_90_150, data_unmasked_150_210, data_unmasked_210_270, data_unmasked_270_330, data_unmasked_330_30]
...
def plane_fit_and_plot(data, data_setting, title, stds, plot = False):
    ...
    if plot is True:
        ...
        ax.scatter(data_setting[' sza'].values, data_setting[' vza'].values, data_setting[' refl_550nm'].values)
        ...
        for i in range(3):
            ...
            axs[i].plot(Y, quad_plane(((i + 1) * 20, Y), *below) - 0.1*Z, color = 'green')
            axs[i].plot(Y, quad_plane(((i + 1) * 20, Y), *above) + 0.1*Z, color = 'orange')
            axs[i].set_title(f'SZA = {(i+1)*20}')
            axs[i].scatter(data_setting[data_setting[' sza'].between((i + 1) * 20 -1, (i + 1) * 20 +1)][' vza'].values,
                           data_setting[data_setting[' sza'].between((i + 1) * 20 -1, (i + 1) * 20 +1)][' refl_550nm'].values,
                           color = 'black', s = 5)
...
outs_19, good_19 = plane_fit_and_plot(data_19_list[0], data_unmasked_list[0], '', 3, plot = False)
for i in range(len(data_19_list) - 1):
    outs_19 = pd.concat([outs_19, plane_fit_and_plot(data_19_list[i+1], data_unmasked_list[i+1], '', 3, plot = False)[0]], ignore_index = True)
    good_19 = pd.concat([good_19, plane_fit_and_plot(data_19_list[i+1], data_unmasked_list[i+1], '', 3, plot = False)[1]], ignore_index = True)
