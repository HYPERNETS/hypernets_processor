...
def interpolate_irradiance_sza(sza, ds_irr, variable):
    ...
    ds_irr_temp[variable].values = (
        ds_irr_temp[variable].values
        / np.cos(np.float64(ds_irr_temp["sza"].values) / 180 * np.pi)[:, None]
    )
    ds_irr_temp = ds_irr_temp.interp(sza=sza, wavelength=wav, method="linear")
    ds_irr_temp[variable].values = ds_irr_temp[variable].values * np.cos(
        np.float64(ds_irr_temp["sza"].values) / 180 * np.pi
    )
...
def sza_vza_bin_and_calc(
    data, cutting_data, stat, bound, input_data=None, input_wav="550"
):
    sza = data[" sza"].values
    vza = data[" vza"].values
    ...
    if stat == "median":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, "median", bins, expand_binnumbers=True
        )
    elif stat == "95th":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile95, bins, expand_binnumbers=True
        )
    elif stat == "5th":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile5, bins, expand_binnumbers=True
        )
    elif stat == "2.5th":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile2_5, bins, expand_binnumbers=True
        )
    elif stat == "97.5th":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile97_5, bins, expand_binnumbers=True
        )
    elif stat == "1st":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile1, bins, expand_binnumbers=True
        )
    elif stat == "99th":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile99, bins, expand_binnumbers=True
        )
    elif stat == "3sigma_min":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile_3sigma_min, bins, expand_binnumbers=True
        )
    elif stat == "3sigma_max":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, percentile_3sigma_max, bins, expand_binnumbers=True
        )
    elif stat == "count":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, "count", bins, expand_binnumbers=True
        )
    elif stat == "std":
        binned_data, x, y, b = binned_statistic_2d(
            sza, vza, refl, "std", bins, expand_binnumbers=True
        )
    elif stat == "median_plus_3std":
        binned_std, x, y, b = binned_statistic_2d(
            sza, vza, refl, "std", bins, expand_binnumbers=True
        )
        binned_median, x, y, b = binned_statistic_2d(
            sza, vza, refl, "median", bins, expand_binnumbers=True
        )
    ...
    if bound == "lower":
        bound_passes = [
            False if v < binned_data[sza_bin_idx[i], vza_bin_idx[i]] else True
            for i, v in enumerate(cutting_refl)
        ]
    elif bound == "upper":
        bound_passes = [
            False if v > binned_data[sza_bin_idx[i], vza_bin_idx[i]] else True
...
def plane_fit_and_plot(
    data, data_setting, stds, plot=True, raa_bin=None, iteration=0, input_wav="550"
):
    x = data[" sza"].values
    y = data[" vza"].values
    ...
    if plot:
        ...
        for i in range(3):
            axs[i].plot(
                Y, quad_plane(((i + 1) * 20, Y), *popt), color="black", alpha=0.1
            )
            axs[i].plot(Y, quad_plane(((i + 1) * 20, Y), *below), color="black")
            axs[i].plot(Y, quad_plane(((i + 1) * 20, Y), *above), color="black")
            ...
            axs[i].scatter(
                outliers[outliers[" sza"].between((i + 1) * 20 - 1, (i + 1) * 20 + 1)][
                    " vza"
                ].values,
                outliers[outliers[" sza"].between((i + 1) * 20 - 1, (i + 1) * 20 + 1)][
                    f" refl_{input_wav}nm"
                ].values,
                color="red",
                s=5,
            )
            ...
            fig.savefig(
                r"T:/ECO/EOServer/joe/hypernets_plots/plane_models/"
                + f"sza_slices_plane_{raa_bin}_{iteration}.png"
            )
            plt.close()
...
class PostProcessingDataset:
    ...
    def calculate_savi(self):
        self.radiance["savi"] = (
            (self.radiance[" refl_842nm"] - self.radiance[" refl_665nm"])
            / (self.radiance[" refl_842nm"] + self.radiance[" refl_665nm"] + 0.5)
        ) * 1.5

    def calculate_s2tci(self):
        self.radiance["s2tci"] = (
            self.radiance[" refl_740nm"] - self.radiance[" refl_705nm"]
        ) / (self.radiance[" refl_705nm"] - self.radiance[" refl_665nm"])

    def split_by_date(self, start_date, end_date):
        self.radiance["datetime"] = [
            datetime.datetime.strptime(x, "%Y%m%d") for x in self.radiance.date
        ]
    ...
    def count_by_bin(self):
        ...
        good_count, sza, vza, b, n = sza_vza_bin_and_calc(
            self.radiance_good,
            self.radiance_good,
            "count",
            "na",
        ...
        out_count, out_sza, out_vza, out_b, out_n = sza_vza_bin_and_calc(
            self.radiance_outliers,
            self.radiance_outliers,
            "count",
            "na",
            input_wav=self.input_wav,
        ...
        percentage_removed = (
            np.true_divide(
                np.array(out_count), (np.array(out_count) + np.array(good_count))
            )
            * 100
        )
        ...
        img = ax.imshow(
            percentage_removed.reshape(15, 15), origin="lower", extent=[0, 75, 0, 75]
        )
    ...
    def append_aod(self):
        if self.site == "GHNA":
            ...
            doy = [
                datetime.date(int(x[0:4]), int(x[4:6]), int(x[6:8])).timetuple().tm_yday
                for x in dates
            ]
            aod_files = [
                aod_path + f"GONA01_{years[i]}_{doy[i]}_v00.09.input"
    ...
    def cloud_check(self, tolerance, wavelength, rewrite, rewrite_var=None):
        ...
        if self.cloud_passes is None:
            data = np.zeros(
                (self.irradiance.shape[0], len(self.irradiance.columns) - 6)
            )
            ...
            for i in range(data.shape[0]):
                aod_1 = find_nearest_to_wav(
                    modelled_data_read_and_interp(
                        szas[i], self.clear_sky_model, "solar_irradiance_BOA"
                    ),
                    wav,
                    wavelength,
                )
            ...
            output_data = np.array(
                [
                    ...
                    observ,
                    abs_diff,
                    [x[12:16] for x in IDs],
                    [x[3:11] for x in IDs],
                    self.irradiance["Flag"],
                ]
            ).T
    ...
    def write_irr_analysis_files(
        self, filename, wavelengths, save=True, plot=False, interp=False
    ):
        szas = self.irradiance["SZA"]
        IDs = self.irradiance["ID"]
        saas = self.irradiance["SAA"]
        ...
        output_data = pd.DataFrame(
            {
                ...
                "SZA": szas,
                "SAA": saas,
                "Time": [x[12:16] for x in IDs],
                "Date": [x[3:11] for x in IDs],
        ...
        for wv in wavelengths:
            ...
            if plot:
                ...
                fig.savefig(
                    r"T:\ECO\EOServer\joe\hypernets_plots\aod_plots\{}_aod_ratio_interpolated_{}.png".format(
                        self.period_name, wv
                    )
                )
        ...
        if save:
            ...
            output_data.to_csv(
                r"T:\ECO\EOServer\data\insitu\hypernets\post_processing_qc\joe\{}.csv".format(
                    filename
                )
            )
    ...
    def plane_fitter(self, labelled=False, plot=False):
        ...
        outs, good = plane_fit_and_plot(
            data_list[0],
            data_unmasked_list[0],
            3,
            plot,
            raa_limits_list[0],
            self.plane_iteration,
        )
    ...
    def new_plane_fitter(
        ...
        for i in range(len(data_list)):
            ...
            if method == "plane_fit":
                ...
                z_lower = quad_plane(
                    (data_cutting[" sza"].values, data_cutting[" vza"].values), *below
                )
                z_upper = quad_plane(
                    (data_cutting[" sza"].values, data_cutting[" vza"].values), *above
                )

                outliers = data_cutting[
                    (data_cutting[f" refl_{self.input_wav}nm"] > z_upper)
                    | (data_cutting[f" refl_{self.input_wav}nm"] < z_lower)
                ]
                good_data = data_cutting[
                    (data_cutting[f" refl_{self.input_wav}nm"] < z_upper)
                    & (data_cutting[f" refl_{self.input_wav}nm"] > z_lower)
                ]
                ...
                std_flat, bin_centers_sza, bin_centers_vza, lower_passes, stds = (
                    sza_vza_bin_and_calc(
                        data_list[i],
                        data_cutting,
                        "std",
                ...
                med_flat, bin_centers_sza, bin_centers_vza, lower_passes, medians = (
                    sza_vza_bin_and_calc(
                        data_list[i],
                        data_cutting,
                        "median",
                ...
                count_flat, bin_centers_sza, bin_centers_vza, lower_passes, counts = (
                    sza_vza_bin_and_calc(
                        data_list[i],
                        data_cutting,
                        "count",
                ...
                df_above = pd.DataFrame(
                    {"refl": upper_bounds, "vza": vza.flatten(), "sza": sza.flatten()}
                )

                lower_interp = LinearNDInterpolator(
                    (sza.flatten(), vza.flatten()), lower_bounds
                )
                lower_passes = [
                    (
                        True
                        if v
                        > lower_interp(data_cutting[" sza"], data_cutting[" vza"])[i]
                        else False
                    )
                    for i, v in enumerate(data_cutting[f" refl_{self.input_wav}nm"])
                ]

                upper_interp = LinearNDInterpolator(
                    (sza.flatten(), vza.flatten()), upper_bounds
                )
                upper_passes = [
                    (
                        True
                        if v
                        < upper_interp(data_cutting[" sza"], data_cutting[" vza"])[i]
                        else False
                    )
                    for i, v in enumerate(data_cutting[f" refl_{self.input_wav}nm"])
                ]
            ...
            if plot:
                ...
                for j in range(3):
                    ...
                    axs[j].scatter(
                        outliers[
                            outliers[" sza"].between((j + 1) * 20 - 1, (j + 1) * 20 + 1)
                        ][" vza"].values,
                        outliers[
                            outliers[" sza"].between((j + 1) * 20 - 1, (j + 1) * 20 + 1)
                        ][f" refl_{self.input_wav}nm"].values,
                        color="red",
                        s=5,
                    )
                    axs[j].scatter(
                        good_data[
                            good_data[" sza"].between(
                                (j + 1) * 20 - 1, (j + 1) * 20 + 1
                            )
                        ][" vza"].values,
                        good_data[
                ...
                fig.savefig(
                    r"T:/ECO/EOServer/joe/hypernets_plots/plane_models/"
                    + "sza_slices_3sig_{}_{}_{}_{}.png".format(
                        raa_limits_list[i], iter_num, method, self.period_name
                    )
                )
                plt.close()
                # plt.show()
        ...
        if nc_path is not None:
            ...
            for i, key in enumerate(ds_dict.keys()):
                ...
                ds_raa_vals_list = [
                    np.mean([float(x) for x in ds_raa_keys_list[i].split("_")])
                    for i in range(len(ds_raa_keys_list))
                ]
    ...
    def plot_ndvi_timeseries(self):
        ...
        fig.savefig(
            root
            + f"/joe/hypernets_plots/QC_pipeline_plots/{self.period_name}_ndvi_timeseries"
        )
...
def JSIT_pipeline(dataset: PostProcessingDataset, maintenance, sza_limit, raa_limit):
    ...
    dataset.save(
        "radiance",
        r"T:\ECO\EOServer\data\insitu\hypernets\post_processing_qc\JSIT_initial_cc9.csv",
    )


def multi_aod_tolerance_analysis(
    dataset: PostProcessingDataset, maintenance, sza_limit, raa_limit
):
    # inital checks
    dataset.maintenance_check(maintenance)
