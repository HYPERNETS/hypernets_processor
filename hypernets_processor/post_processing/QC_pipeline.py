...
class PostProcessingDataset:
    ...
    def cloud_check(self, tolerance, wavelength, rewrite, rewrite_var=None):
        ...
        if self.cloud_passes is None:
            data = np.zeros(
                (self.irradiance.shape[0], len(self.irradiance.columns) - 5)
            )
            ...
            for i in range(data.shape[0]):
                aod_1 = find_nearest_to_wav(
                    modelled_data_read_and_interp(szas[i], self.clear_sky_model),
                    wav,
                    wavelength,
                )
...
QC = PostProcessingDataset(
    r"T:/ECO/EOServer/data/insitu/hypernets/post_processing_qc/GHNA_v1_None_None_None_None.csv",
