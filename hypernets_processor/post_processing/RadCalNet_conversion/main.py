...
from GHNA_processing.data_io.read_hypernets import (
    read_db_hypernets,
    read_hypernets_file,
)
...
def identify_matchups(site, radcalnet_db, start_date, end_date, only_passed_qc=False):
    ...
    for i in range(len(radcat_datetimes)):
        ...
        diff = radcat_datetimes[i] - hypcat_datetimes[id_best_before]
        matchups[i, 3] = diff.days * 24 * 60 + diff.seconds / 60
        diff = radcat_datetimes[i] - hypcat_datetimes[id_best_after]
...
def process_matchup(matchup):
    ...
    cov_hyp_refl_pre = cm.convert_corr_to_cov(
        np.eye(len(u_rand_pre)), u_rand_pre
    ) + cm.convert_corr_to_cov(
        hypernets_ds_pre.err_corr_systematic_reflectance.values, u_syst_pre
    )
    corr_hyp_refl_pre = cm.convert_cov_to_corr(cov_hyp_refl_pre, u_hyp_refl_pre)

    print(
        corr_hyp_refl_pre,
        cm.uncertainty_from_covariance(cov_hyp_refl_pre) / u_hyp_refl_pre,
    )
    ...
    cov_hyp_refl_aft = cm.convert_corr_to_cov(
        np.eye(len(u_rand_aft)), u_rand_aft
    ) + cm.convert_corr_to_cov(
    ...
    rcn_refl_pre = conv.spectral_processing(
        rcn_wav, hyp_wav_pre, hyp_refl_pre, asd_wavs, asd_refl
    )
    u_rcn_refl_pre, corr_rcn_refl_pre = prop.propagate_standard(
        conv.spectral_processing,
        [rcn_wav, hyp_wav_aft, hyp_refl_pre, asd_wavs, asd_refl],
        [None, None, u_hyp_refl_pre, None, u_asd_refl],
        corr_x=[None, None, corr_hyp_refl_pre, None, "rand"],
        return_corr=True,
    )
    rcn_refl_aft = conv.spectral_processing(
        rcn_wav, hyp_wav_aft, hyp_refl_aft, asd_wavs, asd_refl
    )
    u_rcn_refl_aft, corr_rcn_refl_aft = prop.propagate_standard(
        conv.spectral_processing,
        [rcn_wav, hyp_wav_aft, hyp_refl_aft, asd_wavs, asd_refl],
        [None, None, u_hyp_refl_aft, None, u_asd_refl],
        corr_x=[None, None, corr_hyp_refl_aft, None, "rand"],
        return_corr=True,
    )

    rcn_refl_interpolated = conv.time_interpolation(
        hypernets_datetime_pre,
        rcn_refl_pre,
        hypernets_datetime_aft,
        rcn_refl_aft,
        rcn_datetime,
    )  # provide the times and hypernets reflectances
    u_rcn_refl_interpolated = prop.propagate_standard(
        conv.time_interpolation,
        [
            hypernets_datetime_pre,
            rcn_refl_pre,
            hypernets_datetime_aft,
        ...
        [None, u_rcn_refl_pre, None, u_rcn_refl_aft, None],
        corr_x=[None, corr_rcn_refl_pre, None, corr_rcn_refl_aft, None],
        return_corr=False,
    )
    ...
    plt.savefig(
        os.path.join(
            ...
            "plots",
            "comparison_%s.png"
            % (datetime.fromtimestamp(rcn_datetime).strftime("%Y%m%dT%H%M%S")),
        )
    )
    plt.clf()
    ...
    plt.plot(
        rcn_wav,
        (rcn_refl_interpolated - rcn_refl) / rcn_refl * 100,
        label="(GHNA-GONA)/GONA",
    )
    plt.xlabel("Wavelengths (nm)", fontsize=18)
...
def main(start_date, end_date):
    matchups = identify_matchups(
        "GHNA", "GONA_alldata_list_230124.txt", start_date, end_date
    )
    ...
    with open(os.path.join(output_path, "matchups.txt"), "w") as f:
        f.write(
            "# radcalnet_name hypernets_name_before hypernets_name_after timediff_before, timediff_after \n"
        )
    ...
    for ii in range(0, len(output_files)):
        ...
        output_refl_day = np.empty(
            len(rcn_files[np.where(rcn_files == output_files[ii])[0]]), dtype=object
        )
        output_u_refl_day = np.empty(
            len(rcn_files[np.where(rcn_files == output_files[ii])[0]]), dtype=object
        )
        ...
        for iii in range(len(output_refl_day)):
            for i in range(len(matchups)):
                if (
                    matchups[i, 0][0] == output_files[ii]
                    and matchups[i, 0][1] == output_times_day[iii]
                ):
                    (
                        output_refl_day[iii],
                        output_u_refl_day[iii],
                        rcn_wav,
                    ) = process_matchup(matchups[i])
        print(output_refl_day)

        with open(
            os.path.join(output_path, output_files[ii].replace("GONA01", "GHNA01")), "w"
        ) as f:
            f.write("Site:\tGHNA01\n")
