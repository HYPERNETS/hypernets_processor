...
def read_era5_data(era5_mean, era5_spread, S2_time, lon, lat):
    ...
    u_H2O = spread_ds['tcwv'].values[time_min_id[0][0], lat_min_id[0][0], lon_min_id[0][0]]
    P = mean_ds['sp'].values[time_min_id[0][0],lat_min_id[0][0],lon_min_id[0][0]]/100
    u_P = spread_ds['sp'].values[time_min_id[0][0],lat_min_id[0][0],lon_min_id[0][0]]/100
    O3 = mean_ds['tco3'].values[time_min_id[0][0],lat_min_id[0][0],lon_min_id[0][0]]/(2.1415*10**(-5))
    u_O3 = spread_ds['tco3'].values[time_min_id[0][0],lat_min_id[0][0],lon_min_id[0][0]]/(2.1415*10**(-5))
...
def read_aeronet_data(aeronet_data, S2_time):
    ...
    for t in range(len(aerosol_data['Time(hh:mm:ss)'])):
        date = datetime.datetime.strptime(aerosol_data['Date(dd:mm:yyyy)'][t], "%d:%m:%Y").date()
        time = datetime.datetime.strptime(aerosol_data['Time(hh:mm:ss)'][t], '%H:%M:%S').time()
    ...
    aerosol = aerosol_data['AOD_500nm'][time_min_id]*(550/500)**-aerosol_data['500-870_Angstrom_Exponent'][time_min_id]
    print("aod",aerosol,aerosol_data['AOD_500nm'][time_min_id],aerosol_data['500-870_Angstrom_Exponent'][time_min_id])
    u_aerosol = 0.002
...
def read_cams_data(cams_data,S2_time,lon,lat):
    ...
    aerosol = aerosol_ds['aod550'].values[time_min_id[0][0],lat_min_id[0][0],lon_min_id[0][0]]
