...
class Convert_RadCalNet:
    ...
    def convert_datetime(
        ...
        if isinstance(date_time, np.ndarray):
            return np.array(
                [self.convert_datetime(date_time_i) for date_time_i in date_time]
            )
    ...
    def correct_to_nadir(self, ds_hyp):
        ...
        HCRF_model = brdf_model.return_HCRF_for_angle(
            sza=sza,
            vza=vza,
            vaa=vaa,
            saa=saa,
            brdf_params=self.brdf_params.get_values().T,
        )
        HCRF_model_nadir = brdf_model.return_HCRF_for_angle(
            sza=sza,
            vza=np.zeros_like(vza),
            vaa=vaa,
            saa=saa,
            brdf_params=self.brdf_params.get_values().T,
        )
        ds_hyp.reflectance.values = ds_hyp.reflectance.values * cm.interpolate_1d(
            self.irr_all.wavelength.values,
            (HCRF_model_nadir / HCRF_model),
    ...
    def spectral_processing(
        self,
        radcalnet_wavelength,
        hypernets_wavelengths,
        hypernets_reflectance,
        asd_wavelengths,
        asd_reflectances,
    ):
        ...
        hyp_mean_last10nm = np.mean(
            hypernets_reflectance[
                np.where(
                    (hypernets_wavelengths > 1670) & (hypernets_wavelengths < 1680)
                )
            ]
        )
        asd_mean_10nm = np.mean(
            asd_reflectances[
                np.where((asd_wavelengths > 1670) & (asd_wavelengths < 1680))
            ]
        )
        ...
        combined_wavelengths = np.append(
            hypernets_wavelengths[np.where(hypernets_wavelengths < 1680)],
            asd_wavelengths[np.where(asd_wavelengths > 1680)],
        )
        combined_reflectances = np.append(
            hypernets_reflectance[np.where(hypernets_wavelengths < 1680)],
            correction_factor * asd_reflectances[np.where(asd_wavelengths > 1680)],
        )
    ...
    def time_interpolation(
        ...
        refl_interpolated = cm.interpolate_1d(
            time_measured,
            refl_measured,
            time_target,
            u_y_i=None,
