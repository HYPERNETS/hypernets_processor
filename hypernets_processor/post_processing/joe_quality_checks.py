...
def get_outliers(data, sigmas):
    ...
    outliers = data[
        (data[" refl_550nm"] > max_range) | (data[" refl_550nm"] < min_range)
    ]
    good_data = data[
        (data[" refl_550nm"] < max_range) & (data[" refl_550nm"] > min_range)
    ]
    ...
    for i in range(3):
        ...
        axs[i].scatter(
            data[data[" sza"].between((i + 1) * 20 - 1, (i + 1) * 20 + 1)][
                " vza"
            ].values,
            data[data[" sza"].between((i + 1) * 20 - 1, (i + 1) * 20 + 1)][
                " refl_550nm"
...
def cut_and_plot_2d(
    ...
    norm = colors.Normalize(
        vmin=np.min(np.append(out_cuts, good_cuts)),
        vmax=np.max(np.append(out_cuts, good_cuts)),
    )
    images.append(
        axs[0].imshow(
            out_cuts,
            ...
            origin="lower",
            extent=[cut2_ran[0], cut2_ran[1], cut1_ran[0], cut1_ran[1]],
            norm=norm,
        )
    )
    images.append(
