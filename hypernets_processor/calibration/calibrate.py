...
class Calibrate:
    ...
    def calibrate_l1a(self, measurandstring, dataset_l0, dataset_l0_bla, swir=False):
        ...
        cov_systematic_input_qty_indep, cov_systematic_input_qty_corr = self.find_u_systematic_input(input_vars,
                                                                                                     dataset_l0,
                                                                                                     calibration_data)
    ...
    def prepare_calibration_data(self, measurandstring, swir=False):
        hypstar = str(self.context.get_config_value("hypstar_cal_number"))
        ...
        calibration_data["u_systematic_indep_gains"] = gains[:, 2] * (gains[:, 6] ** 2 +
                                                                      gains[:, 7] ** 2 + gains[:, 8] ** 2 + gains[:,
                                                                                                            9] ** 2 + gains[
                                                                                                                      :,
                                                                                                                      10] ** 2 + gains[
                                                                                                                                 :,
                                                                                                                                 11] ** 2 +
                                                                      gains[:, 12] ** 2 + gains[:, 13] ** 2 + gains[:,
                                                                                                              14] ** 2 + gains[
                                                                                                                         :,
                                                                                                                         15] ** 2 +
                                                                      gains[:, 16] ** 2 + gains[:, 17] ** 2 + gains[:,
                                                                                                              19] ** 2) ** 0.5 / 100
        ...
        cov_other = punpy.convert_corr_to_cov(np.eye(len(gains[:, 2])),
                                              gains[:, 2] * (gains[:, 8] ** 2 + gains[:, 9] ** 2 + gains[:,
                                                                                                   11] ** 2 + gains[:,
                                                                                                              16] ** 2 + gains[
                                                                                                                         :,
                                                                                                                         17] ** 2) ** 0.5 / 100)
        ...
        cov_full = punpy.convert_corr_to_cov(np.ones((len(gains[:, 2]), len(gains[:, 2]))),
                                             gains[:, 2] * (gains[:, 7] ** 2 + gains[:, 10] ** 2 + gains[:,
                                                                                                   12] ** 2 + gains[:,
                                                                                                              13] ** 2 + gains[
                                                                                                                         :,
                                                                                                                         14] ** 2 + gains[
    ...
    def find_u_systematic_input(self, variables, dataset, ancillary_dataset):
        ...
        for var in variables:
            try:
                ...
                covs_indep.append(punpy.convert_corr_to_cov(dataset["corr_systematic_" + var].values,
                                                            dataset["u_systematic_" + var].values))
            except:
                try:
        ...
        for i in range(len(input_quantities)):
            ...
            if u_systematic_input_quantities_indep[i] is not None:
                if len(u_systematic_input_quantities_indep[i].shape) < len(datashape):
                    u_systematic_input_quantities_indep[i] = np.tile(u_systematic_input_quantities_indep[i],
                                                                     (datashape[1], 1)).T
            if u_systematic_input_quantities_corr[i] is not None:
                if len(u_systematic_input_quantities_corr[i].shape) < len(datashape):
                    u_systematic_input_quantities_corr[i] = np.tile(u_systematic_input_quantities_corr[i],
                                                                    (datashape[1], 1)).T
