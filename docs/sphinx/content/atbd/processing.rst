...
the raw data taken by the HYPERNETS instruments to reflectances products for both the water and land networks.
The HYPERNETS systems have been deployed at various sites, and collect measurements periodically and and send these to the HYPERNETS servers for processing. 
...
Next, we will discuss the processing that is performed at each processing stages.
...
Then reflectances are calculated in the last stage (L2A) of the processing (together with water leaving radiances for water network). 
...
The last subsection informs how the processing can be configured using various keywords. 
